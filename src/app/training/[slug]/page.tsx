...
const TrainingViewPage: React.FC = () => {
  ...
  const loadTrainingData = async () => {
    try {
      ...
      const response = await contentAPI.getTrainingBySlug(slug);
      console.log('Training data received:', response);
      
      setTraining(response.training);
      setComments(response.comments || []);
    } catch (error: unknown) {
      console.error('Error loading training:', error);
      const errorMessage = error instanceof Error ? error.message : 'Ошибка загрузки тренировки';
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      await contentAPI.createTrainingComment(training!.slug, commentContent.trim());
      setCommentContent('');
      loadTrainingData(); // Reload to show new comment
    } catch (error: unknown) {
  ...
  const getLevelBadge = (level: string) => {
    const badges: Record<string, { bg: string; text: string; label: string }> = {
      beginner: { bg: 'bg-green-100', text: 'text-green-700', label: 'Начальный' },
      intermediate: { bg: 'bg-yellow-100', text: 'text-yellow-700', label: 'Средний' },
      advanced: { bg: 'bg-red-100', text: 'text-red-700', label: 'Продвинутый' }
    };
  ...
  if (error || !training) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 min-h-screen">
        ...
        <Link href="/trainings" className="inline-block mt-4 text-blue-600 hover:text-blue-800">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 min-h-screen">
      {/* Breadcrumb */}
      <div className="mb-4">
        <Link href="/trainings" className="text-blue-600 hover:text-blue-800 text-sm">
          ← Все тренировки
        </Link>
      </div>

      {/* Training Header - matching forum design */}
      <article className="bg-white border-2 border-gray-200/50 mb-4 rounded-sm">
        ...
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
          <div className="flex items-center gap-3 mb-3">
            ...
            {training.durationMinutes && (
              <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium">
                ⏱️ {formatDuration(training.durationMinutes)}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            <span>Автор: <strong>{training.authorName}</strong></span>
            <span>•</span>
