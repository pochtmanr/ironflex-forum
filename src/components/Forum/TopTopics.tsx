              ...
              <li key={topic.id} className="flex justify-between gap-x-6 py-5">
                <div className="flex min-w-0 gap-x-4">
                  ...
                  <div className="min-w-0 flex-auto">
                    <Link to={`/topic/${topic.id}`} className="text-sm/6 font-semibold text-gray-900 hover:text-blue-700">
                      {topic.title}
                    </Link>
                    <p className="mt-1 truncate text-xs/5 text-gray-500">
                      от <Link to={`/profile/${topic.user_id}`} className="text-green-600 hover:text-green-800">@{topic.user_name}</Link>
                    </p>
                  </div>
                </div>
                <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                  <p className="text-sm/6 text-gray-900">В категории <Link to={`/category/${topic.category_id}`} className="text-gray-600 hover:text-gray-800">@{topic.category_name}</Link></p>
