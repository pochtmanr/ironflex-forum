  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Check cache first (cache key includes sort option)
        const cacheKey = `trainings-list-${sortBy}`;
        const cachedData = apiCache.get(cacheKey);
        if (cachedData) {
          console.log('DEBUG: Trainings page - using cached data for', sortBy);
          setItems(cachedData);
          setLoading(false);
          return;
        }

        const res = await contentAPI.getTrainings(1, 50); // Get more trainings for better sorting
        ...
        // Cache for 3 minutes
        apiCache.set(cacheKey, sortedTrainings, 3);
